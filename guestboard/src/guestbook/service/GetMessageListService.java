 ...
 *  MessageDao의 selectCount()로 전체 메시지 개수를 구한다.
 *  요청한 페이지 번호에 해당하는 메시지의 시작 행과 끝 행을 구한다.
 *  MessageDao의 selectList()로 시작행과 끝행에 포함된 메시지 목록을 구한다.
 *  MessageListView객체를 반환한다.
 */
...
public class GetMessageListService {
	...
	public MessageListView getMessageList(int pageNumber){
		...
		try{
			...
			throw new ServiceException("목록구하기실패: "+e.getMessage(),e);
		} finally {
			JdbcUtil.close(conn);
		}
	}
